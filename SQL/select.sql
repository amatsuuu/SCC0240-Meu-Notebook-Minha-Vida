...
SELECT M.NOME, M.CPF, JUNC.PORCENTAGEM_EMPRESTIMO AS "PORCENTAGEM DE EMPRESTIMOS EM 2021", JUNC.QTD
    FROM MONITOR M
	LEFT JOIN (
        SELECT MTRA.MONITOR, ROUND(COUNT(E.ALUNO) / COUNT(*), 2) AS PORCENTAGEM_EMPRESTIMO, COUNT(*) AS QTD
        FROM MONITORIA MTRA
        JOIN TURMA T
        ON MTRA.TURMA = T.ID
